...
rai::Graph getDescriptor(KOMO &komo, int verbose)
{
  ...
  rai::Graph G;
  rai::Array<rai::Node *> collFrames = {};
  rai::Node *coll_node = nullptr;
  for (shared_ptr<Objective> &ob : komo.objectives)
  {
    if (ob->feat->frameIDs.N <= 2 && ob->feat->order <= 1)
    {
      for (GroundedObjective *go : ob->groundings)
      {
        ...
        str o_name;
        o_name << go->feat->typeString() << '_' << go->type << go->feat->order;
        if (go->feat->target.N)
          o_name << '_' << go->feat->target;
        if (verbose > 1)
          cout << "\n  feature:" << o_name;
        if (o_name.contains("PairCollision_ineq"))
        {
          continue;
          coll_node = G.findNode(o_name);
          if (!coll_node)
          {
            coll_node = G.add(o_name, true); // NEW FACTOR
          }
          for (rai::Frame *f : go->frames)
          {
            str f_name;
            f_name << f->name << '_' << f->time;
            if (verbose > 1)
              cout << "\n  frame:" << f_name;
        ...
        for (rai::Frame *f : go->frames)
        {
          ...
          if (!f_node)
          {
            rai::Array<double> size = {0., 0., 0.};
            if(f->getShapeType() == rai::ST_ssBox){
              size = f->getSize();
              // get first three entries of the array
              rai::Array<double> size = {size(0), size(1), size(2)};
            }
            str label = STRING("{pos:" << f->getPosition() << ", " << size << "}");
            f_node = G.add<str>(f_name, label); // NEW VARIABLE
          }
  ...
  for (rai::Node *f : collFrames)
  {
    coll_node->addParent(f);
  }
