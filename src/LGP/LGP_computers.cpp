...
void rai::LGPcomp_Waypoints::untimedCompute() {
  ...
  if(!isComplete && sket->verbose()>4) {
    ...
    if(sket->verbose()>5) komoWaypoints->view_play(true);
  }
  ...
  if(isComplete) {
    if(ret->ineq>.5 || ret->eq>2.) {
      ...
      if(sket->verbose()>3) komoWaypoints->view_play(true);
      //    if(sket->verbose()>1){
      //      static int count=0;
...
rai::LGPcomp_OptimizePath::LGPcomp_OptimizePath(rai::LGPcomp_Skeleton* _sket)
  ...
  if(sket->verbose()>3) komoPath->view_play(true, 0, .1);
...
void rai::LGPcomp_OptimizePath::untimedCompute() {
  ...
  if(!isComplete && sket->verbose()>2) {
    ...
    if(sket->verbose()>3) {
      if(sket->verbose()>5) komoPath->view_play(true, 0, .1);
  ...
  if(isComplete) {
    ...
    if(sket->verbose()>1) komoPath->view_play(sket->verbose()>1 && sol.ret->feasible);
    ...
    if(!sol.ret->feasible) {
      ...
      if(sket->verbose()>0) { //save video and path and everything
        ...
        komoPath->view_play(true, 0, .1, path);
